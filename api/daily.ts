...
export const daily = async (env: any) => {
  try {
    ...
    for (let i = 0; i < 5; i++) {
      try {
        ...
        fanartURL = "https://webservice.fanart.tv/v3.2/music/{0}?api_key={1}".replace('{0}', artists[i].mbid).replace('{1}', env.FANART_API)
    ...
    const resp = await fetch(postUrl, {
      ...
      headers: {
        ...
        'Authorization': 'Bearer ' + env.HUGO_PARAMS_AIRTABLETOKEN
      },
